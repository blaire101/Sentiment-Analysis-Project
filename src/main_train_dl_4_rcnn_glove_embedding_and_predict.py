...
if __name__ == '__main__':
    ...
    # 3. model
    rnn_cnn_model = model.get_rnn_cnn_model(max_num_words=constant.MAX_NUM_WORDS,
                                            max_len=constant.MAX_LEN,
                                            embedding_dim_size=constant.EMBED_SIZE,
                                            embedding_matrix=embedding_matrix)

    # 4. train
    filepath = config.model_path + "rcnn-epoch-{epoch:02d}-val_acc-{val_acc:.4f}.hdf5"
    ...
    logger.info("complete rcnn model trained!")

    # 5. predict

    model_name = config.best_model_path + 'rcnn-04-0.8167.hdf5'

    best_rnn_cnn_model = load_model(model_name)
    ...
    y_pred_rnn_cnn.to_csv(config.predict_result_path + '/y_pred_rnn_cnn.csv', index=False)

    y_pred_rnn_cnn = pd.read_csv(config.predict_result_path + '/y_pred_rnn_cnn.csv')
    ...
    show_metrics_result(y_test, y_pred_rnn_cnn, "test")
