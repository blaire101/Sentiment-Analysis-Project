...
if __name__ == '__main__':
    ...
    # 3. model
    elmo_like_model = model.get_elmo_like(max_num_words=constant.MAX_NUM_WORDS,
                                            max_len=constant.MAX_LEN,
                                            embedding_dim_size=constant.EMBED_SIZE,
                                            embedding_matrix=embedding_matrix)

    # 4. train
    filepath = config.model_path + "elmo-like-ep-{epoch:02d}-val_acc-{val_acc:.4f}.hdf5"

    checkpoint = ModelCheckpoint(filepath, monitor='val_acc', verbose=1,
                                 save_best_only=True, mode='max')


    history = elmo_like_model.fit(x=padded_train_sequences,
                                y=y_train,
    ...
    # # 5. predict code
    #
    # model_name = config.best_model_path + 'elmo-like-epoch-02-val_acc-0.8000.hdf5'
    #
    # best_elmo_like_model = load_model(model_name)
    #
    # logger.info("start predict test data")
    # y_pred_elmo_like = best_elmo_like_model.predict(padded_test_sequences, verbose=1, batch_size=128)
    #
    # y_pred_elmo_like = pd.DataFrame(y_pred_elmo_like, columns=['prediction'])
    #
    # y_pred_elmo_like['prediction'] = y_pred_elmo_like['prediction'].map(lambda p: 1 if p >= 0.5 else 0)
    #
    # y_pred_elmo_like.to_csv(config.predict_result_path + '/y_pred_elmo_like.csv', index=False)
    #
    # y_pred_elmo_like = pd.read_csv(config.predict_result_path + '/y_pred_elmo_like.csv')
    #
    #
    ...
    # # 6. metrics code
    #
    # show_metrics_result(y_test, y_pred_elmo_like, "test")

# =====test run result=====:
#
# test f1_score: 0.789702110869994
# test precision_score: 0.7907898965002171
# test recall_score: 0.7898686679174485